const Popularposts = () => {
  return (
    <div className="sidebar-widget">
      <h3 className="sidebar-widget-title mb-2">Popular posts</h3>
      <div className="sidebar-widget-content">
        <ul className="sidebar-widget-list-post">
          <li>
            <a href="blog-details.html">10 Essential Bodyweight Exercises</a>
          </li>
          <li>
            <a href="blog-details.html">
              How to Keep Workouts Fresh in the New Year
            </a>
          </li>
          <li>
            <a href="blog-details.html">
              15 RD-Approved Foods to Support a Healthy Immune System
            </a>
          </li>
          <li>
            <a href="blog-details.html">Self-care And Stress Management</a>
          </li>
          <li>
            <a href="blog-details.html">Why Do I Need a Health Coach?</a>
          </li>
        </ul>
      </div>
    </div>
  );
};

export default Popularposts;
